...
{
    if (flavour == BEAM_UNRAVELLING && debuff_player(true))
        is_explosion = true;
...
void bolt::tracer_affect_monster(monster* mon)
{
    ...
    if (flavour == BEAM_UNRAVELLING && debuff_monster(mon, true))
        is_explosion = true;
...
bool bolt::nasty_to(const monster* mon) const
{
    ...
    {
        monster mhack = *mon;
        return debuff_monster(&mhack, true);
    }
