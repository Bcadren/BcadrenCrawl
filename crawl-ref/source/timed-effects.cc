...
static void _hell_effects(int /*time_delta*/)
{
    ...
    if (have_passive(passive_t::resist_hell_effects)
        && x_chance_in_y(apply_pity(you.piety), MAX_PIETY * 2) || is_sanctuary(you.pos()))
    {
...
void monster::timeout_enchantments(int levels)
{
    ...
    for (auto &entry : ec)
    {
        switch (entry.first)
        {
        case ENCH_POISON: case ENCH_MAGIC_CANDLE:
        ...
        case ENCH_SILVER_CANDLE: case ENCH_DAZED: case ENCH_FAKE_ABJURATION:
