...
bool drain_wands()
{
    ...
    for (int i = 0; i < ENDOFPACK; ++i)
    {
        ...
        if (you.inv[i].base_type == OBJ_WANDS)
        {
            ...
            {
                you.inv[i].charges -=
                    min(1 + random2(wand_charge_value(you.inv[i].sub_type)),
                        charges);
...
static bool _pakellas_drain_evokers()
{
    ...
    for (int i = 0; i < ENDOFPACK; ++i)
    {
        ...
        if (is_xp_evoker(you.inv[i]))
        {
            if (num_xp_evokers_inert(you.inv[i]) < you.inv[i].quantity
                && coinflip())
            {
                you.inv[i].evoker_debt = you.inv[i].quantity * XP_EVOKE_DEBT;
