...
public:
    ...
    explicit ChaCha(uint32_t* seedval, uint64_t stream = 0);
    template<class Sseq> explicit ChaCha(Sseq& seq);

    void seed(uint32_t* seedval, uint64_t stream = 0);
...
template<size_t R>
inline ChaCha<R>::ChaCha(uint32_t* seedval, uint64_t stream) {
    seed(seedval, stream);
}
...
template<size_t R>
inline void ChaCha<R>::seed(uint32_t* seedval, uint64_t stream) {
    ctr = 0;
    if (seedval == nullptr) {
      return;
    }
    keysetup[0] = seedval[0];
    keysetup[1] = seedval[1];
    keysetup[2] = seedval[2];
    keysetup[3] = seedval[3];
    keysetup[4] = seedval[4];
    keysetup[5] = stream & 0xffffffffu;
    keysetup[6] = stream >> 32;
    keysetup[7] = 0xdeadbeef;      // Could use 128-bit stream.
}
